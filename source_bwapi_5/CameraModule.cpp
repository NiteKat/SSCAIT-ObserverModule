...
CameraModule::CameraModule(BWAPI::Game& game, BWAPI::BWAPIProtoClient& bwapiProtoClient) :
  cameraFocusUnit(nullptr)
{
	...
	followUnit = false;
  Broodwar = &game;
  connectionToServer = &bwapiProtoClient;
}
...
bool CameraModule::shouldMoveCamera(int priority) {
	bool isTimeToMove = Broodwar->getFrameCount() - lastMoved >= cameraMoveTime;
	bool isTimeToMoveIfHigherPrio = Broodwar->getFrameCount() - lastMoved >= cameraMoveTimeMin;
...
void CameraModule::moveCameraArmy() {
	...
	BWAPI::Unit bestPosUnit(nullptr);
	int mostUnitsNearby = 0;

	for each (BWAPI::Unit unit1 in Broodwar->getAllUnits()) {
		if (!isArmyUnit(unit1)) {
			continue;
		}
		...
		int nrUnitsNearby = 0;
		for each (BWAPI::Unit unit2 in Broodwar->getUnitsInRadius(uPos, radius)) {
			if (!isArmyUnit(unit2)) {
				continue;
			}
			nrUnitsNearby++;
...
void CameraModule::updateCameraPosition() {
	...
	if (followUnit && Broodwar->isValid(cameraFocusUnit->getPosition())) {
	...
	if (Broodwar->isValid(currentCameraPosition)) {
    ...
    newSetScreenPosition->set_x(currentMovedPosition.x);
    newSetScreenPosition->set_y(currentMovedPosition.y);
    connectionToServer->queueMessage(std::move(newMessage));
	}
}
